...
Options:
  ...
  --cert-dir PATH     Authoritative generated certificate directory.
...
for service in "${SERVICES[@]}"; do
  case "${service}" in
    gateway|users-service|products-service|media-service|payments-service|eureka-server|prometheus) ;;
    *)
      echo "[remote-cert-volumes] Unsupported service: ${service}" >&2
      exit 2
      ;;
  esac
done
...
if [[ ! -d "${CERT_DIR}" ]]; then
  echo "[remote-cert-volumes] Certificate directory does not exist: ${CERT_DIR}" >&2
  echo "[remote-cert-volumes] Generate certificates first with: bash scripts/generate-certs.sh" >&2
  exit 1
fi
...
TRUSTSTORE_SRC=""
if is_selected gateway || is_selected users-service || is_selected products-service || \
   is_selected media-service || is_selected payments-service || is_selected eureka-server || \
   is_selected prometheus; then
...
GATEWAY_CERT_SRC=""
if is_selected gateway || is_selected prometheus; then
...
USERS_CERT_SRC=""
if is_selected users-service; then
...
PRODUCTS_CERT_SRC=""
if is_selected products-service; then
...
MEDIA_CERT_SRC=""
if is_selected media-service; then
...
PAYMENTS_CERT_SRC=""
if is_selected payments-service; then
...
EUREKA_CERT_SRC=""
if is_selected eureka-server; then
...
[[ -n "${GATEWAY_CERT_SRC}" ]] && install -m 600 "${GATEWAY_CERT_SRC}" "${STAGE_DIR}/certs/gateway.p12"
[[ -n "${USERS_CERT_SRC}" ]] && install -m 600 "${USERS_CERT_SRC}" "${STAGE_DIR}/certs/users-service.p12"
[[ -n "${PRODUCTS_CERT_SRC}" ]] && install -m 600 "${PRODUCTS_CERT_SRC}" "${STAGE_DIR}/certs/products-service.p12"
[[ -n "${MEDIA_CERT_SRC}" ]] && install -m 600 "${MEDIA_CERT_SRC}" "${STAGE_DIR}/certs/media-service.p12"
[[ -n "${PAYMENTS_CERT_SRC}" ]] && install -m 600 "${PAYMENTS_CERT_SRC}" "${STAGE_DIR}/certs/payments-service.p12"
[[ -n "${EUREKA_CERT_SRC}" ]] && install -m 600 "${EUREKA_CERT_SRC}" "${STAGE_DIR}/certs/eureka-server.p12"
...
ssh "${SSH_ARGS[@]}" "${SSH_TARGET}" \
  "HELPER_IMAGE='${HELPER_IMAGE}' bash '${REMOTE_DIR}/setup-cert-volumes.sh' --cert-dir '${REMOTE_DIR}/certs' ${SERVICE_ARGS[*]} ${CLEAN_ARG[*]}"
